...
from src.utils.config_utils import generate_peft_config
from src.utils.train_utils import print_module_size, print_model_size
...
def setup_llm(train_config, model_config, **kwargs):
    ...
    if (train_config.enable_fsdp or train_config.enable_ddp) and train_config.low_cpu_fsdp:
        ...
        if rank == 0:
            ...
            llama_config = AutoConfig.from_pretrained(model_config.llm_path, device_map="auto")
...
def setup_encoder_projector(train_config, model_config, **kwargs):
    if model_config.encoder_projector == "linear":
        ...
        from src.models.projector import EncoderProjectorCov1d
        encoder_projector = EncoderProjectorCov1d(model_config)
    elif model_config.encoder_projector == "q-former":
        from src.models.projector import EncoderProjectorQFormer
...
class stt_model(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.metric = kwargs.get("metric", "wer")
    ...
    def forward(self,
        ...
        if self.metric:
            with torch.no_grad():
                ...
                wer = compute_wer(preds.detach(), labels.detach(), ignore_label=-100)
